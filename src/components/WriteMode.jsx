...
function Summary({ total, correct, wrongAnswers, onRestart, onRetryWrong, t, language }) {
  ...
  return (
    <div className="grid gap-6 xl:grid-cols-[280px_minmax(0,1fr)] animate-float-in">
      <Card className="self-start border-[#CAE8BD] bg-[#ECFAE5] shadow-lg dark:border-slate-700/60 dark:bg-slate-800/90">
        ...
        <CardContent className="space-y-4">
          <div className="rounded-3xl bg-green-50/60 p-5 text-center dark:bg-slate-700">
            ...
            <p className="mt-2 text-lg font-semibold text-green-700">{pct}% {t.correct}</p>
          </div>
      ...
      <Card className="border-[#CAE8BD] bg-[#ECFAE5] shadow-lg dark:border-slate-700/60 dark:bg-slate-800/90">
        <CardHeader>
          ...
          {wrongAnswers.length === 0 ? (
            ...
            <div className="max-h-96 space-y-3 overflow-y-auto pr-1">
              {wrongAnswers.map(({ item, typed }) => (
                <div key={item.id} className="rounded-3xl border border-green-100 bg-white p-4 dark:border-slate-600 dark:bg-slate-700">
...
export default function WriteMode({ vocabulary, language = 'en', t }) {
  ...
  return (
    <Card ref={cardRef} className="overflow-hidden border-[#CAE8BD] bg-[#ECFAE5] shadow-lg animate-float-in dark:border-slate-700/60 dark:bg-slate-800/90">
      <CardHeader className="space-y-4 border-b border-[#CAE8BD] bg-[#ECFAE5] dark:border-slate-700 dark:bg-slate-800/70">

        {/* Status bar */}
        <div className="flex items-center gap-2">
          <span className="rounded-full bg-green-100 px-3 py-1 text-xs font-semibold text-green-700 dark:bg-slate-700 dark:text-slate-300">
            {t.writeTab}
          </span>
          <span className="rounded-full bg-green-100/70 px-3 py-1 text-xs font-semibold text-green-700 dark:bg-slate-700 dark:text-slate-300">
            {t.score}: {score.correct}/{score.total}
          </span>
          <span className="ml-auto rounded-full bg-green-100/70 px-4 py-1 text-sm font-semibold text-green-700 dark:bg-slate-700 dark:text-slate-300">
            {index + 1} / {activeQuestions.length}
          </span>
          <div className="group relative">
            <button
              type="button"
              className="flex h-5 w-5 items-center justify-center rounded-full bg-slate-200 text-xs font-bold text-slate-500 hover:bg-green-200 hover:text-green-700 dark:bg-slate-600 dark:text-slate-400 dark:hover:bg-slate-500"
              aria-label="How to use"
            >
              ?
            </button>
            <div className="pointer-events-none absolute right-0 top-7 z-10 w-56 rounded-2xl border border-green-100 bg-white p-3 text-xs text-slate-600 opacity-0 shadow-lg transition-opacity duration-200 group-hover:opacity-100 group-focus-within:opacity-100 dark:border-slate-600 dark:bg-slate-800 dark:text-slate-300">
              <p className="font-bold text-green-700 dark:text-green-400">How to use</p>
        ...
        {/* Question */}
        <p className="text-2xl font-black text-green-700 dark:text-primary sm:text-3xl">
          {getItemMeaning(item, language)}
        </p>
      </CardHeader>

      <CardContent className="bg-[#ECFAE5] p-4 sm:p-6 dark:bg-slate-800/90">
        <form onSubmit={handleSubmit} className="space-y-4">

          {/* Answer row */}
          <div className={cn(
            'flex items-center gap-3 rounded-2xl border-2 px-4 py-3 transition-colors',
            !submitted && 'border-green-200 bg-white dark:border-slate-600 dark:bg-slate-700',
