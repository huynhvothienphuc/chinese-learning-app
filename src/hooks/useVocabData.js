...
export function prefetchVocab(queryClient, bookId, sectionFile) {
  return queryClient.prefetchQuery({
    queryKey: ['vocab', bookId, sectionFile],
    queryFn: () => fetchDirect(`/data/books/${bookId}/${sectionFile}`).then(parseVocab),
    staleTime: 0,
    gcTime: 1000 * 60 * 30,
  });
}
...
export function useBooks(userId, authReady) {
  return useQuery({
    ...
    queryFn: async () => {
      const [booksData, teacherResult] = await Promise.all([
...
export function useVocabulary(bookId, sectionFile, section) {
  return useQuery({
    ...
    enabled: !!bookId && !!sectionFile && !!section && section.source !== 'upload',
    staleTime: 0,
    gcTime: 1000 * 60 * 30, // keep in memory 30 min — user will revisit sections
...
export function usePrefetchAdjacentSections(bookId, sectionFile, sections) {
  ...
  useEffect(() => {
    ...
    [sections[idx - 1], sections[idx + 1]]
      ...
      .forEach((s) => prefetchVocab(queryClient, bookId, s.file));
