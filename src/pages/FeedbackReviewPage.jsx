...
function FeedbackList({ items, loading, error, filter, onToggleResolved, onDelete, togglingId, deletingId }) {
  ...
  if (displayed.length === 0) {
    return (
      <div className="rounded-2xl border border-dashed border-border bg-white px-6 py-16 text-center text-muted-foreground dark:bg-slate-800">
      ...
      {displayed.map((item, index) => (
        <Card
          ...
          className={cn(
            ...
            item.resolved
              ? 'border-border bg-slate-50 opacity-60 dark:border-slate-800 dark:bg-slate-900'
              : 'border-border bg-card'
          )}
        >
          <CardContent className="flex items-start gap-4 p-4">
            <span className="mt-0.5 flex h-6 w-6 shrink-0 items-center justify-center rounded-full bg-slate-100 text-xs font-bold text-muted-foreground dark:bg-slate-700 ">
              {index + 1}
            </span>
            ...
            <div className="min-w-0 flex-1 space-y-1">
              ...
              {item.chinese && (
                <div className="flex flex-wrap items-center gap-1.5 pb-1">
                  ...
                  {item.book_id && (
                    <span className="rounded-full bg-slate-100 px-2 py-0.5 text-xs text-muted-foreground dark:bg-slate-700 ">
                      {item.book_id}
                    </span>
                  )}
                  {item.section_id && (
            ...
            <button
              ...
              className={cn(
                ...
                item.resolved
                  ...
                  : 'text-slate-300 hover:bg-green-50 hover:text-emerald-500  dark:hover:bg-emerald-900/20 dark:hover:text-emerald-400'
              )}
            ...
            <button
              ...
              className="shrink-0 rounded-lg p-1.5 text-slate-300 transition-colors hover:bg-rose-50 hover:text-rose-500 disabled:opacity-40  dark:hover:bg-rose-900/20 dark:hover:text-rose-400"
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 to-slate-100 px-4 py-10 dark:from-slate-900 dark:to-slate-800">
      <div className="mx-auto max-w-3xl space-y-6">
        ...
        {/* Title card + tabs */}
        <Card className="border-border bg-white shadow-sm dark:bg-slate-800">
          <CardHeader className="border-b border-border pb-4">
            <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
              ...
              {/* Filter tabs */}
              <div className="flex items-center gap-1 rounded-xl border border-border bg-slate-50 p-1 dark:bg-slate-900">
                {FILTERS.map((f) => (
                  <button
                    key={f.key}
                    type="button"
                    ...
                    className={cn(
                      ...
                      filter === f.key
                        ? 'bg-white shadow-sm text-foreground dark:bg-slate-700 dark:text-white'
                  ...
                  >
                    ...
                    <span className={cn(
                      ...
                      filter === f.key ? 'bg-green-100 text-green-700' : 'bg-slate-200 text-muted-foreground dark:bg-slate-700 '
                    )}>
                      {active.counts[f.key]}
                    </span>
                  </button>
