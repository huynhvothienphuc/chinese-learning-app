      ...
      <div className="mx-auto flex min-h-[calc(100vh-2rem)] max-w-3xl flex-col gap-6">
        ...
        <div className="flex items-center justify-between rounded-3xl border border-theme-border bg-theme-surface px-5 py-4 shadow-soft">
          <div className="flex items-center gap-3">
            <button type="button" onClick={() => navigate('/')}>
              <img src="/logo.svg" alt="Logo" className="h-9 w-9 rounded-2xl border border-border bg-white p-1 shadow-sm dark:border-slate-600 dark:bg-slate-700" />
            </button>
        ...
        <div className="flex items-center justify-between">
          <h2 className="text-2xl font-black text-foreground">
            Teachers
            <span className="ml-2 text-base font-normal text-muted-foreground">({teachers.length})</span>
          </h2>
            ...
            <CardContent className="p-5">
              <form onSubmit={addTeacher} className="flex flex-col gap-3">
                ...
                <input
                  ...
                  className="rounded-lg border border-border bg-white px-3 py-2 font-mono text-sm outline-none focus:border-green-400 focus:ring-2 focus:ring-green-100 disabled:opacity-50 dark:border-slate-600 dark:bg-slate-700 dark:text-white"
              ...
              <Card key={teacher.user_id} className="group">
                <CardContent className="flex items-center gap-4 p-5">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-xs text-muted-foreground font-mono truncate">{teacher.user_id}</p>
                  </div>
                  ...
                  <div className="flex items-center gap-2 shrink-0">
                    <span className={`hidden sm:block rounded-full px-2.5 py-0.5 text-xs font-semibold ${
                      teacher.is_active
                        ...
                        : 'bg-slate-100 text-muted-foreground dark:bg-slate-700 dark:text-muted-foreground'
                    }`}>
